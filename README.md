...
A separate editing example retains **Lifes Changes by FNDEF** (rap, 18.46 seconds; MoisesDB validation split). A Three.js view allows dragging the voice while all other objects remain fixed. Direction snaps to 78 SAF-rendered grid points (azimuth -90 to +90 in 15-degree steps; elevation -40 to +60 in 20-degree steps), plus the exact predicted direction. Distance and gain are continuous normalized controls in [0,1]; display radius is not a metric distance. Width and diffuseness stay at their predicted values. The predicted scene is the reset state, and voice-only and loop controls are provided.

The editor lazily loads 44.1 kHz PCM24 lossless FLAC bases and mixes `accompaniment + gain * ((1-distance)*voice_d0 + distance*voice_d1)` in Web Audio. This matches the candidate renderer's affine distance weights up to quantization; it is not an arbitrary room-model interpolation. Moving direction swaps only the voice buffers at the same playhead. The accompaniment is not restarted. No waveform crossfades between HRTF directions, per-setting EQ, loudness matching or live model inference are used. Hard direction changes and un-faded loop boundaries can click. `editor.json` records all 159 basis hashes and export validation. The older discrete-edit WAVs remain in the catalog for reproducibility, but no longer drive the editor UI.
...
All 63 public WAVs were imported byte-for-byte from the validated export, with content-addressed filenames and one common peak-safety scalar per passage. The interactive editor adds 159 rendered FLAC bases with the same safety gain. Its baseline reconstruction RMS error is 6.82e-8; the 4x-oversampled endpoint peak bound is 0.360, with no clipping at intermediate distance/gain settings. These are numerical checks, not perceptual claims. No model inference or training was repeated. The formal listening-study stimuli, responses and paper evaluation results are unchanged and use their original protocol. Current playback provenance is in `demo.json:renderer_update` and `editor.json`; older export/source hashes and explicitly labeled previous-renderer diagnostics remain historical.
